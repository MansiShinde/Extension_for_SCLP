...
bool Name_Ast::is_a_scalar()
{
	...
	cout<<"is a scalar?"<<se.is_a_scalar()<<"\n";
...
{
	// In case of arrays, the indirection level actually represents the number of dimensions.
	// Instead of creating a different function (and then put it up in the base class Ast,
	// I have chosen to reuse this name for arrays.
	
	CHECK_INVARIANT((typeid(*base) == typeid(Name_Ast)), "Wrong type of child in Deref Ast");
...
{
	CHECK_INVARIANT(bn!=NULL, "Base Deref Ast of Deref AST cannot be NULL");
