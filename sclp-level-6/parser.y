	...
	: NAME COCO NAME
		{
			 if (command_options.construct_ast())
			{
				CHECK_INPUT_AND_ABORT(program_object.proc_exists(*$3) == true, "class's function cannot be defined without been declared", yylineno);
				*$$ = *$1 + "_" + *$3;;	
			}
		}
	| NAME
...
func_header
	...
	| func_name
		{
			if (command_options.construct_ast())
				current_procedure = create_function_header($1, no_data_type);
		}
;
	...
	| NAME
		{
			if(program_object.class_name_exists(*$1))
				{
					current_class = program_object.get_class_entry(*$1);
				}
				$$ = class_data_type;
		}
;
...
variable_as_operand
	...
	| variable_name DOT variable_name
		{
			if(command_options.construct_ast())
			{
				//Symbol_Table_Entry  ste = $1->get_symbol_entry();
				//$$ = new Field_Access_Ast($1, $3,*ste.get_class_type_info());
...
Procedure * create_function_header(string * name_ptr, Basic_Data_Type dt)
{
	...
	if(program_object.proc_exists(proc_name))
	{
		...
		//CHECK_INPUT_AND_ABORT(cur_proc->is_proc_defined()==false, 
				//"Current procedure "+proc_name+" already defined", yylineno);
	...
	else
	{
		if(recent_type_name == no_data_type )
				{
    					 CHECK_INPUT_AND_ABORT((current_class->get_class_name() == proc_name), 
"Constructor name "+proc_name+ " should be same as that of its class "+current_class->get_class_name() + " else the function should have some return type", yylineno);
				}
