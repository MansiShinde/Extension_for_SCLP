...
RTL_For_TAC & Asgn_TAC_Stmt::gen_rtl()
{
	...
	*/
	...
	RTL_For_TAC & load_stmt = opd1->create_load_rtl_list();
	...
	RTL_For_TAC store_stmt = result->create_store_rtl_list(load_register);
	...
	//CHECK_INVARIANT (result->is_variable(), "We expect the result of COPY_TAC to be variable, always");
...
RTL_For_TAC & If_Goto_TAC_Stmt::gen_rtl()
{
	...
	RTL_For_TAC & opd_rtl_list = opd1->create_load_rtl_list(); 

	list<RTL_Stmt *> & rtl_list = opd_rtl_list.get_rtl_list();

	Register_Descriptor * dest_register_id = opd_rtl_list.get_reg();
...
RTL_For_TAC & Return_TAC_Stmt::gen_rtl()
{
	...
	RTL_For_TAC & load_stmt = opd1->create_load_rtl_list(proc->get_return_register());
...
RTL_For_TAC & Call_TAC_Stmt::gen_rtl()
{
	...
	for(i = actual_param_list.rbegin(); i != actual_param_list.rend(); i++)
	{
		...
		RTL_For_TAC & load_stmt = (*i)->create_load_rtl_list();
...
RTL_For_TAC & IO_TAC_Stmt::gen_rtl_for_write()
{
	...
	if(opd1->get_basic_data_type() == int_data_type) 
	    ...
	    load_stmt = (opd1->create_load_rtl_list(arg_register)).get_first_rtl_stmt();
...
RTL_For_TAC & Variable_TAC_Opd::create_load_rtl_list()		
{
	...
	*/
	...
	load_stmt->set_asm_comment("Source:" + get_operand_string());
...
RTL_For_TAC & Variable_TAC_Opd::create_store_rtl_list(Register_Descriptor * source_register)
{
	...
	store_stmt->set_asm_comment("Dest: " + get_operand_string());
...
{
	RTL_For_TAC * nop_rtl_code;
	list<RTL_Stmt *> & rtl_list =  *new list<RTL_Stmt *>;
	rtl_list.push_back(new NOP_RTL_Stmt());
	nop_rtl_code = new RTL_For_TAC(rtl_list, NULL);
}
...
RTL_For_TAC & Compute_TAC_Stmt::gen_rtl()
{
	...
	RTL_For_TAC & opd1_load_stmt = opd1->create_load_rtl_list();
	...
	if (is_binary_op(tac_operator))
	{
		...
		RTL_For_TAC & opd2_load_stmt = opd2->create_load_rtl_list();
		reg2 = opd2_load_stmt.get_reg();
		...
		compute_stmt->set_asm_comment(comment + ", Opd1:" + opd1->get_operand_string() + ", Opd2:" + opd2->get_operand_string());

	}
	else
	{
	        compute_stmt = new Compute_RTL_Stmt(rtl_op, reg1_opd, NULL, dest_reg_opd); 
		compute_stmt->set_asm_comment(comment + ", Opd1:" + opd1->get_operand_string());
	}

	//print_tac_stmt(cout);		// FIX: Only for testing.
	//compute_stmt->print_rtl_stmt(cout); // FIX: Only for testing.
