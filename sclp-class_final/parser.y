...
func_call
	...
	| variable_name DOT NAME '(' actual_arg_list ')'
		{
			if(command_options.construct_ast())
			{
				...
				string func_name = current_class->get_class_name() + "_" +  *$3 ; 

				Call_Ast* func_call = new Call_Ast(func_name,yylineno);
        			if($5!=NULL)
   					func_call->set_actual_param_list(*$5);

      				func_call->type_check_actual_formal_param(((program_object.get_procedure_entry(func_name))->get_formal_param_list()));

				
				$$ = new Method_Access_Ast($1, func_call, program_object.get_procedure_entry(func_name)->get_return_type());		
			}
...
field_name
	: NAME 
	{
		if(command_options.construct_ast())
		{
			...
			CHECK_INPUT_AND_ABORT((current_class!=NULL),"Current Class cannot be null",yylineno);		
			...
			if (command_options.construct_ast())
  			{
				...
				else if (current_procedure->is_proc_mangled()) //mangled - to identify its class function 
				{
...
void check_to_add_global_func_in_class(Procedure * cur_proc)
{
	...
	cp->set_proc_is_mangled();
	cp->assign_class_type_to_function(current_class);
