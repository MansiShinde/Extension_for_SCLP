...
TAC_For_Ast * Relational_Expr_Ast::gen_tac()
{
	...
	TAC_Opd * lopd = lhs_res;
	TAC_Opd * ropd = rhs_res;
	TAC_Stmt * relational_stmt = new Compute_TAC_Stmt(get_tac_operator_for_rel_op_in_ast(), lopd, ropd, res_temp, lhs_condition->get_basic_data_type()); 
...
TAC_For_Ast * Logical_Expr_Ast::gen_tac()
{
	...
	TAC_Stmt * relational_stmt = new Compute_TAC_Stmt(get_tac_operator_for_logical_op_in_ast(), lopd, ropd, res_temp, get_basic_data_type()); 
...
//////////////////////////////////////////////////////////////////////////////
	...
	// Overall strategy
	// Add two functions for Field_Access_Ast: 
	// - gen_tac which generates the result in a temporary
	// - gen_tac_aggregate which generates the result as access of field as an operand to appear on LHS
	// gen_tac is used for generating TAC for RHS whereas gen_tac_aggregate is used for generating
	// TAC for aggregate LHS where we want the result to go in an aggregate. Since the RHS of
	// Asgn_TAC_Stmt is ALWAYS a temporary, the LHS can be a aggregate.

TAC_For_Ast * Field_Access_Ast::gen_tac_aggregate()
{
	CHECK_INVARIANT((base != NULL), "Base cannot be null");
	CHECK_INVARIANT((field != NULL), "Offset cannot be null");

	TAC_For_Ast  * base_code = base->gen_tac();
	TAC_For_Ast * field_code = field->gen_tac();

	CHECK_INVARIANT(base_code!=NULL, "Code generated for base of Field AST cannot be NULL");
	CHECK_INVARIANT(field_code!=NULL, "Code generated for field of Field AST cannot be NULL");
	TAC_Opd * field_res = field_code->get_result();
	CHECK_INVARIANT(field_res, "Result of TAC list cannot be null");

	TAC_Opd * base_res = base_code->get_result();
	CHECK_INVARIANT(base_res, "Result of TAC list cannot be null");

	TAC_Opd * field_access_opd = new Field_Access_TAC_Opd(base_res, field_res);
	...
	if (base_code->get_tac_list() != NULL)
        ...
        // Append RHS list to the new list 
	if (field_code->get_tac_list() != NULL)
		tac_list->splice(tac_list->end(), *field_code->get_tac_list());

	TAC_Opd * list_result = field_access_opd;
	TAC_For_Ast * ast_code = new TAC_For_Ast(tac_list, list_result);

	return ast_code;
}

TAC_For_Ast * Field_Access_Ast::gen_tac()
{
	TAC_For_Ast  * field_access_code = gen_tac_aggregate();

	CHECK_INVARIANT(field_access_code!=NULL, "Code generated for of Field AST cannot be NULL");


	TAC_Opd * field_opd = field_access_code->get_result();
	CHECK_INVARIANT(field_opd, "Result of TAC list cannot be null");

	// Get a new temporary 

	TAC_Opd * res_temp = new Temporary_TAC_Opd(field->get_basic_data_type());

	TAC_Stmt * copy_stmt = new Compute_TAC_Stmt(copy_3a, field_opd, NULL, res_temp, field->get_basic_data_type()); 
	...
	CHECK_INVARIANT(tac_list, "TAC list cannot be null");
        ...
        // Copy LHS list to the new list 
	if (field_access_code->get_tac_list() != NULL)
		tac_list = field_access_code->get_tac_list();
