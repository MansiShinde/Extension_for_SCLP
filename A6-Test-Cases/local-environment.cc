...
{
	stringstream msg;
	msg << "No get_double_value() function for " << typeid(*this).name();
	CHECK_INVARIANT(CONTROL_SHOULD_NOT_REACH, msg.str());
}
...
{
	stringstream msg;
	msg << "No set_value(double) function for " << typeid(*this).name();
	CHECK_INVARIANT(CONTROL_SHOULD_NOT_REACH, msg.str());
}
...
{
	stringstream msg;
	msg << "No set_value() function for " << typeid(*this).name();
	CHECK_INVARIANT(CONTROL_SHOULD_NOT_REACH, msg.str());
}
		...
		if (variable_table.find((*i).first) != variable_table.end())
		{
			...
			else
			{
				if (vi->get_result_enum() == int_result)
					...
					file_buffer << VAR_SPACE << (*i).first << " : " << vi->get_double_value() << "\n";
			}
		}
	}
}
