...
TAC_For_Ast * Assignment_Ast::gen_tac()
{
	...
	//cout << "Generated tac for LHS " << endl;    // TEST
	...
	//cout << "Generated  tac statement for assignment " << endl;    // TEST
	...
	//cout << "Obtained  new tac list" << endl;    // TEST

	TAC_Opd * list_result = NULL;
	TAC_For_Ast * ast_code = new TAC_For_Ast(tac_list, list_result);

	//cout << "Generate TAC code for assignment statement" << endl;    // TEST
...
TAC_For_Ast * Selection_Statement_Ast::gen_tac()
{
	...
	//cond_code->print_tac_stmt_list(cout);				// TEST
	...
	//not_cond_stmt->print_tac_stmt(cout);//
	...
	//if_else_stmt->print_tac_stmt(cout);//
	//then_code->print_tac_stmt_list(cout);//

	TAC_Stmt * if_goto_stmt = new Goto_TAC_Stmt(if_goto);	// 3 stmt
	//if_goto_stmt->print_tac_stmt(cout);//

	if (else_part != NULL)
	{
		label1 = new Label_TAC_Stmt(rhs_res);	// 4 stmt
		//label1->print_tac_stmt(cout);	//
		else_code = else_part->gen_tac();
		//else_code->print_tac_stmt_list(cout);	//
		
	}

	label2 = new Label_TAC_Stmt(if_goto);	// 5 stmt
	//label2->print_tac_stmt(cout);//
	/* Make new list */
	list<TAC_Stmt *> * tac_list = new list<TAC_Stmt *>;
	CHECK_INVARIANT(tac_list, "TAC list cannot be null");
...
TAC_For_Ast * Conditional_Expression_Ast::gen_tac()
{
	...
	//cout<<"Tac of ternary generated";			// TEST
	//exit(1);
...
{
	//CHECK_INVARIANT(seq_ast_statement_list, "Sequence list cannot be null");
	TAC_For_Ast * t_code;
...
{
	//cout << "Generating TAC for number " << endl;    // TEST
	...
	//cout << "Generated TAC for number " << endl; 	// TEST

	//list_result->print_opd(cout);    // TEST
	...
	//cout << "Generated TAC for Number AST" << endl; 	// TEST
	return ast_code;
}
