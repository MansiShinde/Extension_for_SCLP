...
class RTL_Insn_Descriptor
{
	...
	*/
	...
	RTL_Op inst_op;
	string mnemonic;
	string rtl_symbol;       /* symbol for printing in RTL code */
	string name;
	RTL_Format rtl_format;   /* format for printing in RTL code */
	ASM_Format assem_format;

public:
	RTL_Insn_Descriptor (RTL_Op op, string name, string mnn, string rtl_str, RTL_Format rtlf, ASM_Format af);
	RTL_Insn_Descriptor();
	~RTL_Insn_Descriptor() {}
...
public:
	Control_Flow_RTL_Stmt(RTL_Op op, RTL_Opd * o1, RTL_Opd * o2, string label, int size = 0);
