...
void Procedure::gen_icode()
{
	...
	list<Ast *>::iterator i;
	for (i = proc_statement_list.begin(); i != proc_statement_list.end(); i++)
	{
		Ast * ast = *i;
		...
		// if (typeid(*ast) != typeid(Return_Ast))
		{
			...
			{
				if (proc_icode_list.empty())
					proc_icode_list = ast_icode_list;
				else
					proc_icode_list.splice(proc_icode_list.end(), ast_icode_list);
			}
		}
	}
}
...
void Procedure::print_icode(ostream & file_buffer)
{
	...
	for (i = proc_icode_list.begin(); i != proc_icode_list.end(); i++)
...
void Procedure::print_assembly(ostream & file_buffer)
{
	...
	for (i = proc_rtl_list.begin(); i != proc_rtl_list.end(); i++)
