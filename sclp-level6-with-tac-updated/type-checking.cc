...
void Assignment_Ast::type_check_ast()
{
	...
	if (lhs->is_an_array())
		CHECK_INPUT_AND_ABORT(lhs_ind_lev==0, "Missing dimension in array access on LHS", lineno);
	if (rhs->is_an_array())
		CHECK_INPUT_AND_ABORT(rhs_ind_lev==0, "Missing dimension in array access on RHS", lineno);

	bool indirection_level_match = (lhs_ind_lev == rhs_ind_lev) && (lhs_ind_lev >= 0);

	CHECK_INPUT_AND_ABORT(type_match, "Assignment statement data type not compatible", lineno);
	CHECK_INPUT_AND_ABORT(indirection_level_match, "Incorrect number of pointer indirections or number of array dimensions in assignment statement", lineno);
}
