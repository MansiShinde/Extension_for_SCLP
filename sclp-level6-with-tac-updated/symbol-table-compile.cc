...
void Symbol_Table::assign_offsets()
{
	...
	for (i = variable_table.begin(); i != variable_table.end(); i++)
	{
		...
		{
			if((*i)->get_basic_data_type() == class_data_type)
			{
				size = (*i)->get_class_obj_size();
			}
				
			else
				size = -(get_size_of_value_type((*i)->get_basic_data_type()));
...
int Symbol_Table::get_size_of_value_type(Basic_Data_Type dt)
{
	return - number_of_bytes_for_value_type(dt);
}
